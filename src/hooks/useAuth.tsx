...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      const authResponse = await startAsync({ authUrl: authUrl });

      if(authResponse.type === 'success' && (authResponse.params.error || authResponse.params.error !== "access_denied")) {
        ...
        const tempUser = userResponse.data.data[0] as User;
        setUser(tempUser);
